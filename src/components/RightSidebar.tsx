    ...
    svg: (
      ...
      >
        ...
        <path
          ...
          d="M128 0C94.06 0 61.48 13.49 37.5 37.49A128.04 128.04 0 0 0 0 128c0 33.93 13.5 66.51 37.5 90.51C61.48 242.51 94.06 256 128 256s66.52-13.49 90.5-37.49c24-24 37.5-56.58 37.5-90.51s-13.5-66.51-37.5-90.51C194.52 13.49 161.94 0 128 0"
        />
        <path
          fill="#fff"
          d="M57.94 126.65q55.98-24.38 74.64-32.15c35.56-14.79 42.94-17.35 47.76-17.44 1.06 0 3.42.25 4.96 1.49 1.28 1.05 1.64 2.47 1.82 3.47.16 1 .38 3.27.2 5.04-1.92 20.24-10.26 69.36-14.5 92.03-1.78 9.59-5.32 12.81-8.74 13.12-7.44.68-13.08-4.91-20.28-9.63-11.26-7.39-17.62-11.98-28.56-19.19-12.64-8.33-4.44-12.91 2.76-20.39 1.88-1.96 34.64-31.75 35.26-34.45.08-.34.16-1.6-.6-2.26-.74-.67-1.84-.44-2.64-.26-1.14.26-19.12 12.15-54 35.69-5.1 3.51-9.72 5.22-13.88 5.13-4.56-.1-13.36-2.58-19.9-4.71-8-2.61-14.38-3.98-13.82-8.41.28-2.3 3.46-4.66 9.52-7.07"
        />
      </svg>
    ),
  },
  ...
  return (
    <div className="flex flex-col items-center text-center w-full">
      ...
      <p className="mb-8 max-w-xs text-sm text-gray-600">
        Join our vibrant developer channels for support, collaboration, and
        updates.
      </p>
      <nav className="flex space-x-6 mb-6">
        {SOCIAL_LINKS.map(({ name, href, svg }) => (
          <a
            ...
            className="text-gray-700 hover:text-[#239165] transition-colors"
            aria-label={name}
          >
      ...
      <nav className="flex space-x-4 text-xs text-gray-400">
        {FOOTER_LINKS.map(({ label, href }) => (
          <a
            key={label}
            href={href}
            className="hover:underline hover:text-[#239165]"
          >
            {label}
          </a>
        ))}
...
export default function ResponsiveSidebar() {
  // scroll progress from 0 (far from bottom) to 1 (at bottom)
  ...
  useEffect(() => {
    function handleScroll() {
      ...
      // Start showing footer smoothly when within 300px of bottom
      if (distanceFromBottom <= 300) {
        progress = 1 - distanceFromBottom / 300;
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex fixed top-1/2 right-3 -translate-y-1/2 w-56 flex-col bg-white py-10 px-6 shadow-md rounded-2xl">
        <SidebarContent />
      </aside>

      {/* Smooth footer on small devices */}
      <footer
        className="fixed bottom-0 left-0 right-0 z-50 flex lg:hidden w-full flex-col items-center py-6 px-4 
         pt-8 mt-8"
